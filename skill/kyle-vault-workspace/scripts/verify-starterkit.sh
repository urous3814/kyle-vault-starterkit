...
for file in "${required_files[@]}"; do
  [ -f "$file" ] || fail "missing required file: ${file#$skill_dir/}"
done
...
grep -q '^description: .*Conductor' "$skill_dir/SKILL.md" ||
  fail "SKILL.md description must mention Conductor"
...
for script in \
  ...
  bash -n "$script" || fail "shell syntax failed: ${script#$skill_dir/}"
done
...
json_check() {
  ...
  if command -v jq >/dev/null 2>&1; then
    ...
    fail "no JSON validator found for ${json_file#$skill_dir/}"
  fi
}
